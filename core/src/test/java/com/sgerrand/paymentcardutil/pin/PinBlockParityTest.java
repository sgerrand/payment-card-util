...
/** Checks pin blocks and pin verification values against the Python cardutil package. */
class PinBlockParityTest {
    ...
    Stream<DynamicTest> iso0MatchesCardutil() {
        return Vectors.tests(
                ...
                testCase -> {
                    ...
                    assertEquals(
                            testCase.get("block_hex").asText(),
                            Vectors.hex(block.toBytes()),
                            "block");
                    assertEquals(
                            testCase.get("encrypted_hex").asText(),
                            Vectors.hex(block.toEncryptedBytes(testCase.get("key").asText())),
                            "encrypted block");

                    if (!testCase.get("pvv").isNull()) {
                        assertEquals(
                                testCase.get("pvv").asText(),
                                block.toPvv(
                                        testCase.get("pvv_key").asText(),
                                        testCase.get("pvv_key_index").asInt()),
                                "pvv");
                    }
                });
    }
    ...
    Stream<DynamicTest> iso0RoundTrips() {
        return Vectors.tests(
                "iso0_pinblock",
                testCase -> testCase.get("pin").asText(),
                testCase -> {
                    String pin = testCase.get("pin").asText();
                    String cardNumber = testCase.get("card_number").asText();
                    ...
                    assertEquals(
                            pin,
                            Iso0PinBlock.fromBytes(
                                            Vectors.hex(testCase.get("block_hex").asText()),
                                            cardNumber)
                                    .pin());
                    assertEquals(
                            pin,
                            Iso0PinBlock.fromEncryptedBytes(
    ...
    Stream<DynamicTest> iso4RoundTrips() {
        return Vectors.tests(
                ...
                testCase -> {
                    ...
                    assertEquals(
                            pin,
                            Iso4PinBlock.fromBytes(Vectors.hex(testCase.get("block_hex").asText()))
                                    .pin());
                    assertEquals(
                            pin,
                            Iso4PinBlock.fromEncryptedBytes(
                                            Vectors.hex(testCase.get("encrypted_hex").asText()),
