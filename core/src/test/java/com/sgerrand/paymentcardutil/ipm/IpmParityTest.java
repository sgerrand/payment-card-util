...
/**
 * Checks IPM and VBS files against files produced by the Python cardutil
 * package.
 */
class IpmParityTest {
    ...
    Stream<DynamicTest> writeMatchesCardutil() {
        return Vectors.cases("ipm").stream().map(testCase -> DynamicTest.dynamicTest(
                testCase.get("name").asText(),
                () -> {
                    ByteArrayOutputStream out = new ByteArrayOutputStream();
                    try (IpmWriter writer = IpmWriter.open(out, options(testCase), blocked(testCase))) {
    ...
    Stream<DynamicTest> readMatchesCardutil() {
        return Vectors.cases("ipm").stream().map(testCase -> DynamicTest.dynamicTest(
                ...
                () -> {
                    ...
                    try (IpmReader reader = IpmReader.open(
                            new ByteArrayInputStream(file), options(testCase), blocked(testCase))) {
    ...
    Stream<DynamicTest> writeVbsMatchesCardutil() {
        return Vectors.cases("vbs").stream().map(testCase -> DynamicTest.dynamicTest(
                testCase.get("name").asText(),
                () -> {
                    ByteArrayOutputStream out = new ByteArrayOutputStream();
    ...
    private static Iso8583Options options(JsonNode testCase) {
        return Iso8583Options.defaults().withCharset(Vectors.charset(testCase.get("encoding").asText()));
    }
