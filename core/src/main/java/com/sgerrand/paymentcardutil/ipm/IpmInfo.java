 ...
 *
 * <p>Useful before reading a file whose blocking and character set nobody recorded:
 *
 * <pre>{@code
...
public record IpmInfo(boolean valid, String reason, boolean blocked, Encoding encoding) {
     ...
     *
     * <p>This is worked out from the message type indicator, which is four digits. That is enough
     * to tell an ASCII compatible file from an EBCDIC one, and no more: the digits sit in the same
     * place in every EBCDIC code page, so cp500 and cp037 cannot be told apart this way. Where the
    ...
    public enum Encoding {
         ...
         *
         * <p>For {@link #EBCDIC} this is a starting point rather than a finding. If the file turns
         * out to be a different code page, digits and plain letters still read correctly;
         * punctuation is where they differ.
         */
    ...
    /**
     * Works out what a file holds from the start of it, checking the bitmap against a given layout.
     */
    public static IpmInfo inspect(byte[] sample, IsoConfig config) {
        ...
        if (recordLength > config.maxVbsRecordLength()) {
            return invalid(
                    "First record says it is "
                            + recordLength
                            + " bytes, past the "
                            + config.maxVbsRecordLength()
                            + " byte limit, which usually means the file is damaged");
        }
    ...
    /**
     * Whether every data element the first bitmap claims is one the layout knows about.
     *
    ...
    /**
     * Whether the file is in 1014 byte blocks, told by the filler bytes on the end of each block.
     *
     * <p>cardutil only reports blocking for files that are exactly one or two blocks long, so it
     * answers no for almost every real file. This checks the blocks in the sample instead, whatever
    ...
    /**
     * Works out the kind of character set from the message type indicator, which is always four
     * digits. Whichever reading gives four digits is the one the file uses.
     */
