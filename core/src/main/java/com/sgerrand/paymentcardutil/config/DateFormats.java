...
/**
 * Turns Python {@code strftime} date patterns, as used in the ISO 8583 field config, into Java date
 * formatters.
 *
 * <p>Two digit years follow Python's rule, not Java's: {@code 00} to {@code 68} mean 2000 to 2068,
 * and {@code 69} to {@code 99} mean 1969 to 1999. Java's own {@code yy} pattern would read every
 * two digit year as 2000 to 2099, which would date 1990s records a century late.
 */
public final class DateFormats {

    /**
     * First year of the 100 year window a two digit year falls in. Matches Python's {@code
     * time.strptime}.
     */
     ...
     *
     * <p>Supported directives: {@code %y %Y %m %d %H %M %S %j %f %%}. Anything else in the pattern
        ...
        for (int i = 0; i < pythonFormat.length(); i++) {
            ...
            if (i + 1 >= pythonFormat.length()) {
                throw new IllegalArgumentException(
                        "Date format ends with a stray %: " + pythonFormat);
            }
    ...
    private static void append(
            DateTimeFormatterBuilder builder, char directive, String pythonFormat) {
        switch (directive) {
