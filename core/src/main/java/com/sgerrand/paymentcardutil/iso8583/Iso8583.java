 ...
 * }</pre>
 *
 * <p>Pass {@link Iso8583Options} for a different character set, a hex bitmap or
 * a different message layout.
 *
 * <p>How many digits and bytes each field takes comes from the layout in
...
public final class Iso8583 {
    ...
    public static Iso8583Message parse(byte[] message, Iso8583Options options) {
        ...
        if (message.length < headerLength) {
            throw new Iso8583Exception(
                    "Message is " + message.length + " bytes, too short for a message type and bitmap",
                    message, null);
        }
        ...
        for (int bit = 2; bit <= MAX_FIELD; bit++) {
            ...
            int de = bit;
            FieldConfig field = options.config().field(de).orElseThrow(() -> new Iso8583Exception(
                    "No layout configured for DE" + de, message, null));
            pointer += readField(de, field, body, pointer, options, builder);
        }

        if (pointer != body.length) {
            throw new Iso8583Exception(
                    "Message data is the wrong length. The bitmap accounts for " + pointer
                            + " bytes, the message holds " + body.length,
                    message, null);
        }
     ...
     * Writes a message.
     *
     * <p>Any {@code PDSxxxx} values are packed into the data elements set up to
    ...
    private static Bitmap readBitmap(byte[] message, Iso8583Options options) {
        if (!options.hexBitmap()) {
            return Bitmap.of(java.util.Arrays.copyOfRange(message, MTI_LENGTH, MTI_LENGTH + BITMAP_LENGTH));
        }
    ...
    private static String readMti(byte[] message, Charset charset) {
        ...
        try {
            ...
            throw new Iso8583Exception("Message type indicator is not a number: " + mti, message, e);
        }
        return mti;
    }
     ...
     */
    private static int readField(int bit, FieldConfig field, byte[] body, int pointer,
                                 Iso8583Options options, Iso8583Message.Builder builder) {
        ...
        if (lengthSize > 0) {
            ...
            try {
                ...
                throw new Iso8583Exception("DE" + bit + " has a length that is not a number: " + lengthText,
                        body, e);
            }
            if (fieldLength < 0) {
                throw new Iso8583Exception("DE" + bit + " has a negative length: " + fieldLength, body, null);
            }
        }
        ...
        byte[] raw = java.util.Arrays.copyOfRange(body, Math.min(start, body.length), Math.max(start, end));
    ...
    private static byte[] writeField(FieldConfig field, Object value, Charset charset) {
        ...
        if (value instanceof byte[] bytes) {
            int length = field.lengthSize() > 0 ? bytes.length : Math.min(field.length(), bytes.length);
    ...
    private static String toText(Object value, FieldConfig field) {
        return switch (field.valueType()) {
            ...
            case DECIMAL -> padLeftZeros(asDecimal(value).setScale(6, java.math.RoundingMode.HALF_UP).toPlainString(),
                    field.length());
    ...
    private static Object toValue(int bit, String text, FieldConfig field) {
        try {
            return switch (field.valueType()) {
                ...
                case DATETIME -> LocalDateTime.parse(text, DateFormats.of(field.dateFormatOrDefault()));
                case TEXT -> text;
            };
        } catch (NumberFormatException | DateTimeParseException e) {
            throw new Iso8583Exception("DE" + bit + " does not hold a " + field.valueType() + ": " + text, e);
        }
    }
    ...
    /**
     * Moves every {@code PDSxxxx} value into the data elements set up to carry
     * them, lowest numbered element first.
     */
        ...
        if (groups.size() > carriers.size()) {
            throw new Iso8583Exception(
                    "The message holds more private data than fits: " + groups.size()
                            + " data elements needed, the layout has " + carriers.size());
        }
     ...
     *
     * <p>Missing and empty values are left out; a zero is written, since zero is
     * a real amount.
     */
