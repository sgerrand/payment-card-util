 ...
 *
 * <p>A tag is one byte, or two if the first byte is {@code 9F} or {@code 5F}. The length is one
 * byte. A tag of {@code 00} marks the end of the data.
 */
final class IccCodec {
    ...
    enum OnError {
        ...
        /**
         * Reads the setting from a {@code field_processor_config} string such as {@code
         * on_error=ERROR}. Anything unrecognised means {@link #WARN}.
         */
     ...
     *
     * @param bit which data element the chip data came from, for error messages
     ...
     * @return {@code ICC_DATA} holding the whole field as hex, plus one {@code TAGxxxx} entry per
     *     tag read
    ...
    static Map<String, String> unpack(int bit, byte[] fieldData, String processorConfig) {
        ...
        while (pointer < fieldData.length) {
            ...
            if (pointer >= fieldData.length) {
                stopOrThrow(
                        onError, bit, "No length byte for tag " + tag + " at position " + pointer);
                break;
            }
