 ...
 *
 * <p>A PVV lets an issuer check a pin without storing it. Card number, key index and pin are joined
 * into a sixteen digit block, encrypted under the verification key, and the first four decimal
 * digits of the result are kept. Where the result has fewer than four decimal digits, the letters
 * are folded down into digits to make the count up.
 *
...
public final class VisaPvv {
    ...
    /**
     * The only pin length a PVV is defined for. The block it is worked out from is 11 card digits,
     * a key index and the pin, and that has to come to exactly 16 digits.
     */
     ...
     * @throws IllegalArgumentException if the pin is not {@value #REQUIRED_PIN_LENGTH} digits, or
    ...
    public static String calculate(String pin, String pvvKey, int keyIndex, String cardNumber) {
        ...
        if (pin.length() != REQUIRED_PIN_LENGTH) {
            throw new IllegalArgumentException(
                    "A pin verification value needs a "
                            + REQUIRED_PIN_LENGTH
                            + " digit pin, this one is "
                            + pin.length());
        }
    ...
    /**
     * Keeps the first four decimal digits of the ciphertext. If there are fewer than four, the
     * letters are used in order, each reduced by ten, until there are enough.
     */
