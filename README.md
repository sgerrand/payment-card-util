...
generated by running the Python package. To rebuild it:

```sh
python -m venv .venv
.venv/bin/pip install 'cardutil[crypto]'
mkdir -p build && cd build          # so the local sources do not shadow the package
../.venv/bin/python ../tools/gen_vectors.py \
    ../core/src/test/resources/vectors/cardutil.json
```

`tools/gen_config.py` regenerates `DefaultConfig.java`, the message layout, the
